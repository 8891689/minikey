...
void bin2hex(unsigned char* buffer, int length); // 将字节数组转换为16进制字符串

void sha256sse_23(uint8_t *src0, uint8_t *src1, uint8_t *src2, uint8_t *src3,
                    uint8_t *dst0, uint8_t *dst1, uint8_t *dst2, uint8_t *dst3);
void sha256sse_31(uint8_t *src0, uint8_t *src1, uint8_t *src2, uint8_t *src3,
                    uint8_t *dst0, uint8_t *dst1, uint8_t *dst2, uint8_t *dst3);
...
int main(int argc, char **argv) {
    ...
    pthread_t tid;
    char c;
    while ((c = getopt(argc, argv, "a:m:s:t:urd")) != -1) {
        switch (c) {
        case 'r':
            FLAG_RANDOM = 1;
            break;
...
void *process_verified22(void *vargp) {
    ...
    do {
        ...
        sha256sse_23((uint8_t*)current_minikey[0], (uint8_t*)current_minikey[1],
                     (uint8_t*)current_minikey[2], (uint8_t*)current_minikey[3],
        ...
        for (i = 0; i < 4; i++) {
            if (keyvalue[i][0] == 0x00) {
                ...
                if (FLAG_HEX) {
                    ...
                    // 计算完整私钥：对有效 mini key（去掉问号）做 SHA256
                    sha256((unsigned char*)current_minikey[i], SIZE_VALUE, privkey);
                    bin2hex(privkey, 32);
                }
                else {
                    fprintf(stdout, "%s\n", current_minikey[i]);
...
void *process_verified30(void *vargp) {
    ...
    do {
        ...
        increment_minikey_index(minikey, raw_minikey, INCREMENT_OFFSET);
        pthread_mutex_unlock(&thread_mutex);
        sha256sse_31((uint8_t*)current_minikey[0], (uint8_t*)current_minikey[1],
                     (uint8_t*)current_minikey[2], (uint8_t*)current_minikey[3],
                     keyvalue[0], keyvalue[1], keyvalue[2], keyvalue[3]);
        for (i = 0; i < 4; i++) {
            if (keyvalue[i][0] == 0x00) {
...
// 将字节数组转换为16进制字符串输出
void bin2hex(unsigned char* buffer, int length) {
    for (int i = 0; i < length; i++) {
        printf("%02x", buffer[i]);
    }
    printf("\n");
